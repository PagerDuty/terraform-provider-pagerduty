...
func testSweepSchedule(_ string) error {
	ctx := context.Background()
	resp, err := testAccProvider.client.ListSchedulesWithContext(ctx, pagerduty.ListSchedulesOptions{})
	if err != nil {
		return err
	}
	...
	for _, schedule := range resp.Schedules {
		if strings.HasPrefix(schedule.Name, "test") || strings.HasPrefix(schedule.Name, "tf-") {
			...
			if err := testAccProvider.client.DeleteScheduleWithContext(ctx, schedule.ID); err != nil {
				return err
			}
		}
	}
...
func TestAccPagerDutySchedule_Basic(t *testing.T) {
	...
	startWrongFormated := testAccTimeNow().Add(24 * time.Hour).Round(1 * time.Hour).Format(time.RFC1123)
	startNotRounded := testAccTimeNow().Add(24 * time.Hour).Round(1 * time.Hour).Add(5 * time.Second).Format(time.RFC3339)
...
func TestAccPagerDutySchedule_WithExternalDestroyHandling(t *testing.T) {
	username := fmt.Sprintf("tf-%s", acctest.RandString(5))
	email := fmt.Sprintf("%s@foo.test", username)
	schedule := fmt.Sprintf("tf-%s", acctest.RandString(5))
	location := "America/New_York"
...
func TestAccPagerDutyScheduleWithTeams_EscalationPolicyDependantWithOpenIncidents(t *testing.T) {
	...
	unrelatedIncidentID := ""
	pUnrelatedIncidentID := &unrelatedIncidentID
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(
						"pagerduty_escalation_policy.foo", "name", escalationPolicy1),
					testAccCheckPagerDutyScheduleOpenIncidentOnService(pIncidentID, "pagerduty_service.foo", "pagerduty_escalation_policy.foo"),
				),
			},
			{
			...
			},
			{
				...
				Config: testAccCheckPagerDutyScheduleWithTeamsEscalationPolicyDependantWithOpenIncidentConfig(username, email, schedule, location, start, rotationVirtualStart, team, escalationPolicy1, service1),
				Check: resource.ComposeTestCheckFunc(
					testAccPagerDutyScheduleResolveIncident(pIncidentID, "pagerduty_escalation_policy.foo"),
				),
			},
			{
				Config: testAccCheckPagerDutyScheduleWithTeamsEscalationPolicyDependantWithOpenIncidentConfigUpdated(username, email, team, escalationPolicy1, service1),
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					testAccCheckPagerDutyScheduleOpenIncidentOnService(pUnrelatedIncidentID, "pagerduty_service.bar", "pagerduty_escalation_policy.bar"),
				),
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(
						"pagerduty_escalation_policy.bar", "name", escalationPolicy2),
					testAccPagerDutyScheduleResolveIncident(pUnrelatedIncidentID, "pagerduty_escalation_policy.bar"),
				),
			},
		},
	})
...
func TestAccPagerDutyScheduleOverflow_Basic(t *testing.T) {
	...
	location := "America/New_York"
	t.Setenv("PAGERDUTY_TIME_ZONE", location)
	start := testAccTimeNow().Add(30 * time.Hour).Round(1 * time.Hour).Format(time.RFC3339)
	rotationVirtualStart := testAccTimeNow().Add(30 * time.Hour).Round(1 * time.Hour).Format(time.RFC3339)
...
func TestAccPagerDutySchedule_WithWeek(t *testing.T) {
	username := fmt.Sprintf("tf-%s", acctest.RandString(5))
	email := fmt.Sprintf("%s@foo.test", username)
	schedule := fmt.Sprintf("tf-%s", acctest.RandString(5))
	scheduleUpdated := fmt.Sprintf("tf-%s", acctest.RandString(5))
...
func TestAccPagerDutySchedule_Multi(t *testing.T) {
	...
	t.Setenv("PAGERDUTY_TIME_ZONE", location)
	start := testAccTimeNow().Add(24 * time.Hour).Round(1 * time.Hour).Format(time.RFC3339)
	end := testAccTimeNow().Add(72 * time.Hour).Round(1 * time.Hour).Format(time.RFC3339)
	rotationVirtualStart := testAccTimeNow().Add(24 * time.Hour).Round(1 * time.Hour).Format(time.RFC3339)
...
func testAccCheckPagerDutyScheduleDestroy(s *terraform.State) error {
	for _, r := range s.RootModule().Resources {
		...
		if _, err := testAccProvider.client.GetScheduleWithContext(ctx, r.Primary.ID, o); err == nil {
...
func testAccCheckPagerDutyScheduleExists(n string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		ctx := context.Background()
		found, err := testAccProvider.client.GetScheduleWithContext(ctx, rs.Primary.ID, pagerduty.GetScheduleOptions{})
		if err != nil {
			return err
		}
...
func testAccExternallyDestroySchedule(n string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		ctx := context.Background()
		err := testAccProvider.client.DeleteScheduleWithContext(ctx, rs.Primary.ID)
		if err != nil {
			return err
		}
...
func testAccCheckPagerDutyScheduleOpenIncidentOnService(p *string, sn, epn string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		resp, err := testAccProvider.client.CreateIncidentWithContext(ctx, "", incident)
		if err != nil {
			return err
		}
...
func testAccPagerDutyScheduleResolveIncident(p *string, _ string) resource.TestCheckFunc {
	return func(_ *terraform.State) error {
		ctx := context.Background()

		incident, err := testAccProvider.client.GetIncidentWithContext(ctx, *p)
		if err != nil {
			return err
		}
		...
		incidentOptions := buildPagerdutyManageIncidentsOptions(incident)

		_, err = testAccProvider.client.ManageIncidentsWithContext(ctx, "", []pagerduty.ManageIncidentsOptions{incidentOptions})
		if err != nil {
			return err
		}
...
func buildPagerdutyManageIncidentsOptions(incident *pagerduty.Incident) pagerduty.ManageIncidentsOptions {
	var priority *pagerduty.APIReference
	...
	var assignments []pagerduty.Assignee
	for _, assign := range incident.Assignments {
		assignments = append(assignments, pagerduty.Assignee{
			Assignee: pagerduty.APIObject{
				ID:   assign.Assignee.ID,
				Type: assign.Assignee.Type,
			},
		})
	}

	return pagerduty.ManageIncidentsOptions{
		ID:          incident.ID,
		...
		EscalationPolicy: &pagerduty.APIReference{
			...
			Type: incident.EscalationPolicy.Type,
		},
		Resolution:       "",
		ConferenceBridge: incident.ConferenceBridge,
	}
