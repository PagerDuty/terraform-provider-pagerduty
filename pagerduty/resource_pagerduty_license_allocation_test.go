		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					//testAccCheckPagerDutyTeamMembershipExistsInStateWithRole("pagerduty_team_membership.foo", firstRole),
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					//testAccCheckPagerDutyTeamMembershipExistsInStateWithRole("pagerduty_team_membership.foo", secondRole),
					resource.TestCheckResourceAttr(
...
func testAccCheckPagerDutyTeamMembershipExistsInStateWithRole(n string, neededRole string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		rs, ok := s.RootModule().Resources[n]
		...
		if strings.Compare(role, neededRole) != 0 {
			return fmt.Errorf("role '%s' from state does not match the needed role of: %s", role, neededRole)
		}
